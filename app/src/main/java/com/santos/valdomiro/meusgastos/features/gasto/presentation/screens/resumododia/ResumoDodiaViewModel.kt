...
import com.santos.valdomiro.meusgastos.features.gasto.domain.usecase.GetResumoDoDiaUseCase
...
@HiltViewModel
class ResumoDodiaViewModel @Inject constructor(
    private val getResumoDoDiaUseCase: GetResumoDoDiaUseCase
) : ViewModel() {

    private val _uiState = MutableStateFlow<UiState<List<GastoEntity>>>(UiState.Idle)
    val uiState = _uiState.asStateFlow()

    private val _resumoState = MutableStateFlow(ResumoDoDiaState())
    ...
    fun getAll(data: LocalDate) {
        viewModelScope.launch {
            ...
            getResumoDoDiaUseCase(data = data)
    ...
    private fun gerarResumo(listaGastos: List<GastoEntity>) {
        ...
        _resumoState.value = ResumoDoDiaState(
